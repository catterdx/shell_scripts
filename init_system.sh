...
CleanUp() {
  if [ "$1" ] && [ "$1" -eq 1 ] && [ -d "$TMP_DIR" ] && cd "$HOME"; then
    rm -rf "$TMP_DIR" && echo -e "${Msg_Info}temp file has benn cleaned."
  elif [ -d "$TMP_DIR" ]; then
...
ChkPriv() {
  if [ $EUID -ne 0 ]; then
    echo -e "\n\n${Msg_Warning}This script must be run as ${Font_Red}root${Font_Suffix}!\n\n"
    Usage
    exit 1
  fi
}
...
ChkPM() {
  if [ "$os_release" = "centos" ]; then
    if CmdExists dnf; then
      ...
      echo -e "${Msg_Info}Package manager command 'dnf' not found."
    fi
  elif [ $os_release = ubuntu ] || [ $os_release = debian ]; then
    if CmdExists apt; then
      echo apt
    else
      echo -e "${Msg_Info}Package manager command 'apt' not found."
    fi
  else
    echo "$os_release"
  fi
...
RPMInPkgs() {
  ...
  if ! grep -qi "fedora" /etc/os-release; then
    pre_pkgs=(langpacks-en dnf-plugins-core epel-release epel-next-release)
    for pre_pack in "${pre_pkgs[@]}"; do
  ...
  dnf makecache
  rpm_pkgs=(langpacks-en man-db man-pages wget curl vim-enhanced zsh bash-completion git tree tar unzip gzip zip bzip2 xz cmake gcc gcc-c++ bc dos2unix iotop tmux netcat nmap-ncat net-tools nmap util-linux-user python3-devel libcurl-devel bind-utils iperf3 htop screen iftop)
  ...
  echo -e "${Msg_Success}Dnf job done."
...
DEBInPkgs() {
  ...
  apt update
  deb_pkgs=(passwd language-pack-en man-db manpages wget curl zsh git virt-what tar unzip zip gzip bzip2 xz-utils tree net-tools netcat ncat iperf3 mtr vim iftop htop iotop screen tmux dos2unix bc python3-pip python3-dev fonts-powerline gcc g++ cmake autoconf)
  echo -e "${Msg_Info}Installing desired packages..."
  ...
  echo -e "${Msg_Success}Apt job done."
}
...
InstallPkgs() {
  if [ "$*" = "dnf" ]; then
    ...
    echo -e "${Msg_Info}Your current os is: $*, supporting OS: CentOS 8+, RockyLinux, AlmaLinux, Ubuntu 14.04+, Debian 8+"
    exit 1
  fi
}
  ...
  if CmdExists pip3; then
    ...
    for py_pack in "${py_pkgs[@]}"; do
      if sudo -i -u "${SUDO_USER:-$USER}" pip3 show -q "$py_pack"; then
        ...
        echo -e "${Msg_Info}Installing pip packages..."
        sudo -i -u "${SUDO_USER:-$USER}" pip3 -q install --user "$py_pack" && echo -e "${Msg_Success}pip3: $Font_Purple$py_pack$Font_Suffix installed successfully." || echo -e "$Msg_Failed\"$py_pack\" can not be installed "
      fi
    done
    echo -e "${Msg_Success}Pip job done."
  fi
}

InstallOhMyZsh() {
  if ! [ -d ~/.oh-my-zsh ]; then
    curl -fsSL "https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh" | sudo -u "${SUDO_USER:-$USER}" bash > /dev/null
    omz_dir_exists=1
  fi
}
...
ZshPlugins() {
  if ! [ -d ~/.oh-my-zsh/custom/plugins/you-should-use ]; then
    ...
    echo -e "${Msg_Success}omz plugins job done."
  fi
}
...
ModifyVimRC() {
  ...
  if [ -f ~/.vimrc ] && ! [ -d ~/.vim ]; then
    if ! grep -Eq "Custom" ~/.vimrc; then
      ...
      git clone https://github.com/dracula/vim.git ~/.vim/pack/themes/start/dracula && echo -e "${Msg_Success}Dracula theme for vim OK!"
      curl -sSfL https://raw.githubusercontent.com/amix/vimrc/master/vimrcs/basic.vim > ~/.vimrc && echo -e "$Msg_Success.vimrc replaced!"
...
UpdateApps() {
  ...
  git_apps_1=(https://github.com/sharkdp/bat
  ...
  for git_app in "${git_apps_1[@]}"; do
    ...
    if CmdExists "$app_name"; then
      if current_version="$("$real_home/.local/bin/$app_name" --version 2> /dev/null)" || current_version="$("$real_home/.local/bin/$app_name"-v 2> /dev/null)" || current_version="$("$app_name" --version 2> /dev/null)" || current_version="$("$app_name" -v 2> /dev/null)"; then
    ...
    for app_tar_name in "${app_tar_names[@]}"; do
      ...
      if Is404 "$download_url"; then
        ...
        curl -SfLO --progress-bar "$download_url" && tar -axf "$app_tar_name" && rm "$app_tar_name" && if CfgGitApps; then echo -e "$Msg_Success\"$app_name\"($latest_ver) installed successfully."; fi || echo -e "$Msg_Failed: unable to download $app_name."
        break
      fi
    done
  done
  ...
  echo -e "${Msg_Success}Github utilities job done."
}

CfgGitApps() {
  sudo -u "${SUDO_USER:-$USER}" mkdir -p ~/.local/share/{bash,zsh}-completion/completions || return
  sudo -u "${SUDO_USER:-$USER}" mkdir -p ~/.local/share/man/man{1..10} || return
...
AddCstmFunc() {
  ...
  echo -e "${Msg_Success}Custom functions job done."
}
...
Usage() {
  cat >&2 <<- EOF
		...
		# | Arguments: [au], Deauflt is '-a'.
		# | -a: Full, deb|dnf install packages
...
main() {
  ...
  if [ "$1" = "-a" ] || [ "$#" -eq 0 ]; then
    ...
    echo -e "${Msg_Error}Wrong argument(s)."
    Usage
    exit 1
  fi
  Tasks "$start_args"
