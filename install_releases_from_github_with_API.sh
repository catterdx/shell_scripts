...
command -v curl &> /dev/null || {
  echo -e "Please install ${Font_Red}curl${Font_Suffix} and try again." >&2
  exit 1
}
command -v tar &> /dev/null || {
  echo -e "Please install ${Font_Red}tar${Font_Suffix} and try again." >&2
  exit 1
}
command -v find &> /dev/null || {
  echo -e "Please install ${Font_Red}find(fildutils)${Font_Suffix} and try again." >&2
  exit 1
}
